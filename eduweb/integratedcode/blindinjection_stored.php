...
// create connection
	...
	$sql = "select * from problemtable where problemid=6";
						...
						<div class="col-md-8">
							<?php
								if(isset($_POST['username']) && isset($_POST['comments']))
								{
									...
									$sql = "insert into usercomments(commentdate,username,usercomments,subject) values(now(),'" . $username . "','". $comments ."', 'SQL feedbackform')";
									//echo $sql;	
									...
									else if(sql_command_check3($username) === true)
									{
										$objectiveresult = "Invalid username";
									}
									else if(sql_command_check4($comments) === true)
									{
										$objectiveresult = "Invalid Comments";
									}
									else
									{		
										...
										else
										{
											...
											if ($showresult->num_rows > 0) {
												echo "<table class=table style='text-align:center;border: 1px solid black;'><tr style='text-align:center;border: 2px solid black;'><th style='text-align:center;border: 2px solid black;'> Username </th style='text-align:center;border: 2px solid black;'><th style='text-align:center;border: 2px solid black;'> Comments </th><th style='text-align:center;border: 2px solid black;'> DATE </th><th style='text-align:center;border: 2px solid black;'> SUBJECT </th></tr>";
												while($row = $showresult->fetch_assoc()) {
													...
													echo "<td style='text-align:center;border: 1px solid black;'>" . $row["usercomments"] . "</td>";
													echo "<td style='text-align:center;border: 1px solid black;'>" . $row["commentdate"] . "</td>";
													echo "<td style='text-align:center;border: 1px solid black;'>" . $row["subject"] . "</td>";
													echo "</tr>";
												}
												echo "</table>";
											}
									...
									'date' => date("Y-m-d"),
									'problemid' => $problemid,
									'problemname' => 'SQLInjection(stored)',
									'problemstatus' => $objectiveresult
									);
									curl_logactivity($data);
									$_SESSION["logactivity"] = $_SESSION['uniquekey']." ".date("Y-m-d")." ".$objectiveresult." "."SQLInjection(stored)";
									$conn->close();
								}
