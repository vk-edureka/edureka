...
// create connection
	...
	$sql = "select * from problemtable where problemid=15";
	...
	if(isset($_POST['userlogin']) && isset($_POST['userpwd']))
	{
		...
		if ($_POST['usercaptcha'] == $_SESSION["captcha"])
		{
			...
			{
				$statusmessage = "<font color=green>login Successful. you found the captcha. welcome ".$username."<br>your answer has been recorded. proceed to next question or attempt</font> ";
				$objectiveresult = "success";
			}
			else
			{
				$statusmessage = "<font color=red>Invalid username or password</font>";
				$objectiveresult = "Not success";
			}
		}
		else
		{
			$statusmessage = "<font color=red>Invalid captcha</font>";
			$objectiveresult = "Not success";
		}
		$data = array(
		...
		'date' => date("Y-m-d"),
		'problemid' => $problemid,
		'problemname' => 'ba_searchcaptcha',
		'problemstatus' => $objectiveresult
		);
		curl_logactivity($data);
		$_SESSION["logactivity"] = $_SESSION['uniquekey']." ".date("Y-m-d")." ".$objectiveresult." "."ba_searchcaptcha";
	}
							...
							<form class="register-form" method="post" onsubmit="return TryToLogin()">
								<div class="row" style="padding: 30px;">						
									<div class="form-group" style="align: center;">
										...
										<div class="row">
											<input type="hidden" name="mapcaptcha" id="mapcaptcha" value="<?php echo $_SESSION["captcha"]; ?>" />
										</div>
